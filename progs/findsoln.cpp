...
int main( int argc, char *argv[] ){
 ...
 MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  ...
  if (rank != 0 ) { 
    ...
    std::cerr.rdbuf (sink->rdbuf());
     ...
     const Real dtmin          = args.getreal("-Dtmin",  "--mintimestep", 1e-7, "minimum time step");
 ...
 newtonHookstepSearch ( searchflags, T, dtmax, dtmin, timestep, log, Nx, Ny, Lx, Ly, D, Ra, Be, Pr, Cc, precondfPsi, precondfTheta, precondfZeta, solnum, filename, ofilepath, ofilename, rank, norm_max, norm_l2, ax, verbose, amp) ;
